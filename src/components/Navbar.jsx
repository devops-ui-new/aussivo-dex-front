...
export default function Navbar() {
  ...
  useEffect(() => {
    setNavOpen(false);
    setMenuOpen(false);
  }, [pathname]); // close menus on navigation
    ...
    <>
      <header className="relative z-50">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {isLoggedIn ? (
              <div className="relative">
                ...
                >
                  {account ? (
                    <div className="h-8 w-8 shrink-0 overflow-hidden rounded-lg ring-1 ring-white/10 bg-surface-4">
                      <Avatar
                        seed={account}
                        style="bottts"
                        size={32}
                  ...
                  <div className="hidden sm:block text-left">
                    <div className="text-sm font-medium font-display leading-tight">
                      {user?.name || "User"}
                    </div>
                    <div className="flex items-center gap-1 -mt-0.5">
                      {account && (
                        <span
                          className={`inline-flex items-center gap-1 text-[9px] font-semibold ${chainId === CHAIN.id ? "text-emerald-400" : "text-amber-400"}`}
                        >
                          <span
                            className={`h-1.5 w-1.5 rounded-full ${chainId === CHAIN.id ? "bg-emerald-400" : "bg-amber-400"}`}
                          />
                          {chainId === CHAIN.id ? CHAIN.short : "Wrong network"}
                        </span>
                      )}
                      <span className="text-[10px] text-muted font-mono">
                        {user?.email && !account ? user.email : short}
                      </span>
                    </div>
                  </div>
                </button>
                {menuOpen && (
                  <div
                    className="absolute right-0 mt-2 w-60 glass p-2 shadow-2xl"
                  ...
                  >
                    <div className="px-3 py-2.5 mb-1 border-b border-surface-4/50">
                      ...
                      {user?.referralCode && (
                        <div className="text-[10px] text-brand font-mono font-bold mt-1">
                      ...
                      {account && (
                        <div className="mt-2 flex items-center gap-2">
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="ml-auto inline-flex items-center gap-1 text-[10px] text-slate-400 hover:text-brand"
                          >
                            explorer
                            <svg
                    ...
                    <Link
                      to="/portfolio"
                      className="flex items-center gap-2.5 px-3 py-2 text-sm text-slate-300 hover:bg-surface-3 rounded-lg"
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-red-400 hover:bg-surface-3 rounded-lg mt-1 border-t border-surface-4/50 pt-2"
                    >
                      ⏏ Sign out
                    </button>
                  </div>
              ...
              <button
                ...
                disabled={connecting}
                className="btn-primary !rounded-full !px-4 sm:!px-6 !py-2.5 !text-[0.8rem] !font-semibold tracking-wide disabled:opacity-60 disabled:cursor-not-allowed"
              >
        ...
        {navOpen && (
          <>
            <div
              className="md:hidden fixed inset-0 top-[74px] z-40 bg-black/50 backdrop-blur-sm"
              onClick={() => setNavOpen(false)}
            />
            <nav
              className="md:hidden absolute inset-x-0 top-[74px] z-50 border-t border-surface-4/60 bg-surface-1/95 backdrop-blur-xl shadow-2xl"
              style={{ animation: "navDrop .18s ease-out" }}
            >
              <div className="max-w-7xl mx-auto px-4 py-3">
                {NAV.map((n) => {
                  ...
                  return (
                    ...
                    >
                      {n.label}
                      <svg
                        className={`h-4 w-4 ${active ? "text-brand" : "text-slate-500"}`}
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="2.2"
                ...
                {isLoggedIn && account && (
                  <div className="mt-2 flex items-center gap-2 border-t border-surface-4/50 px-4 pt-3 text-[11px] text-muted">
                    ...
                    <span
                      className={`ml-auto font-semibold ${chainId === CHAIN.id ? "text-emerald-400" : "text-amber-400"}`}
                    >
                      {chainId === CHAIN.id ? CHAIN.short : "Wrong network"}
                    </span>
                  </div>
